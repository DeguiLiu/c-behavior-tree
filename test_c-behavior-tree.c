...
static const bt_case_t g_cases[] =
    {
        {"Basic Sequence", test_basic_sequence, "Basic SEQUENCE + SELECTOR flow"},
...
static void cmd_bt_status(int argc, char **argv)
{
    ...
    rt_kprintf("  counter=%u, progress=%u, flag=%u\n",
               (unsigned)g_ctx.counter, (unsigned)g_ctx.progress, (unsigned)g_ctx.flag);
